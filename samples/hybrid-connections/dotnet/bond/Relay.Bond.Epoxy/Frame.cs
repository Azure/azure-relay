...
namespace Relay.Bond.Epoxy
{
    ...
    class Frame
    {
        ...
        const int DefaultFrameCount = 3;
        readonly List<Framelet> framelets;
        ...
        public async Task WriteAsync(Stream stream)
        {
            ...
            var binaryWriter = new BinaryWriter(memStream, Encoding.UTF8, true);
        ...
        static async Task<byte[]> ReadBufferAsync(Stream stream, int length, CancellationToken ct)
        {
            ...
            while (bytesToRead > 0)
            {
                ...
                if (dataRead == 0)
                {
                    throw new EndOfStreamException("End of stream encountered while reading frame");
                }
