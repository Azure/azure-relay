...
namespace Relay.Bond.Epoxy
{
    ...
    static class EpoxyProtocol
    {
        // This needs to be larger than the longest valid path through the state machine.
        static readonly uint maximumTransitions = (uint) Enum.GetNames(typeof (ClassifyState)).Length;

        static readonly Deserializer<FastBinaryReader<InputBuffer>> configDeserializer =
            new Deserializer<FastBinaryReader<InputBuffer>>(typeof (EpoxyConfig));

        static readonly Deserializer<FastBinaryReader<InputBuffer>> headersDeserializer =
            new Deserializer<FastBinaryReader<InputBuffer>>(typeof (EpoxyHeaders));

        static readonly Deserializer<FastBinaryReader<InputBuffer>> errorDeserializer =
            ...
            Logger logger)
        {
            ...
            logger.Site().Debug(
                "Deserialized {0} with conversation ID {1} and message type {2}.",
                nameof(EpoxyHeaders),
        ...
        internal static ClassifyState TransitionExpectOptionalLayerData(
            ...
            ref ArraySegment<byte> layerData,
            ref ProtocolErrorCode? errorCode,
            Logger logger)
        {
            ...
            if (framelet.Type == FrameletType.LayerData)
            {
                ...
                logger.Site().Debug(
                    "Extracted {0}-byte layer data in conversation ID {1}.",
                    layerData.Count,
                    headers.conversation_id);
            }
